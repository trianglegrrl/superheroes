 ...
 *
 * Invoke with an id and attribute and it will capitalize the attribute name and wrap it in span
 *  which provides the tooltip
 *
 * Invoke with an id, attribute and a label and it will wrap the label in the tooltipped span
 * The id acts as a semaphore, causing the helper to refresh
 *
 * Use makeBoundHelper here rather than registerBoundHelper as per appkit. Using makeBoundHelper
 * seems to blow up.
 *
 * FIXME (or at least UNDERSTANDME)
 * Specifying the 'id' as a final argument to makeBoundHelper should cause it to refresh when id changes -
 * why doesn't it work? This would allow us to not pass id as an argument to the 
 */
export default Ember.Handlebars.makeBoundHelper(function (similarityMatrix, attribute, label) {
